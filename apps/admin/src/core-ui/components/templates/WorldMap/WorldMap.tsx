...
export const WorldMap = ({
  ...
  const {
    ...
    token,
    lockPeriod,
  } = useNetworkConfigStore((store) => store);
  ...
  return (
    ...
    >
      ...
      {showVaquitasListModal && (
        <VaquitasListModal
          open={showVaquitasListModal}
          onOpenChange={() => setShowVaquitasListModal(false)}
        />
      )}
      {showBankAPYModal && (
        <BankAPYModal open={showBankAPYModal} onOpenChange={() => setShowBankAPYModal(false)} />
      )}
    </div>
  );
};

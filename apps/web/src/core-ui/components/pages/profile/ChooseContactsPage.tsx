...
export function ChooseContactsPage() {
  ...
  const toggleInvite = (contact: Contact) => {
    setInvited((prev) => {
      ...
      if (next.has(contact.id)) {
        ...
        toast.success(t('social.contacts.removedToast', { name: contact.name }), { timeout: 1500 });
      } else {
        next.add(contact.id);
        toast.success(t('social.contacts.invitedToast', { name: contact.name }), { timeout: 1500 });
      }
      return next;
    });
  };
  ...
  return (
    <MockedSubPageLayout
      ...
      subtitle={t('social.contacts.subtitle')}
    ...
    >
      ...
      <div className="relative">
        ...
        <input
          type="search"
          inputMode="search"
          placeholder={t('social.contacts.searchPlaceholder')}
          value={query}
      ...
      <section className="flex flex-col gap-3">
        <div className="flex items-center justify-between px-1">
          <h2 className="text-xs font-extrabold uppercase tracking-wider text-gray-500">
            {t('social.contacts.listHeading')}
          </h2>
          <span className="text-[11px] font-bold text-gray-500 tabular-nums">
            {t('social.contacts.foundCount', { count: filtered.length })}
          </span>
        </div>
        {filtered.length === 0 ? (
          <div className="rounded-2xl border border-black border-b-2 bg-white p-6 text-center">
            <p className="text-sm font-bold text-black">{t('social.contacts.noMatchesTitle')}</p>
            <p className="text-xs text-gray-500 mt-1">{t('social.contacts.noMatchesBody')}</p>
          </div>
        ) : (
                  ...
                  >
                    {isInvited ? (
                      <>
                        ...
                        {t('social.contacts.invited')}
                      </>
                    ) : (
                      <>
                        <FiUserPlus className="h-3.5 w-3.5" />
                        {t('social.contacts.invite')}
                      </>
                    )}
                  </button>
                </li>
